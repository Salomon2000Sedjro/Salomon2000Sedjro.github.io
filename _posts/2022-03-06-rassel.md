...
<img src="http://latex.codecogs.com/svg.latex?\mathcal{G}{RASSEL}&space;=&space;{\widehat{g}^{(1)},...,\widehat{g}^{(l)},...,\widehat{g}^{(L)}}and&space;forming&space;the&space;ensemble&space;prediction&space;function&space;as:\widehat{f}^{(L)}(.)&space;=&space;\dfrac{1}{L}\sum{l=1}^{L}\widehat{g}^{(l)}(.)" title="http://latex.codecogs.com/svg.latex?\mathcal{G}{RASSEL} = {\widehat{g}^{(1)},...,\widehat{g}^{(l)},...,\widehat{g}^{(L)}}and forming the ensemble prediction function as:\widehat{f}^{(L)}(.) = \dfrac{1}{L}\sum{l=1}^{L}\widehat{g}^{(l)}(.)" />
