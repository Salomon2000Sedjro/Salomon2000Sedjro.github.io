...
![formula](https://render.githubusercontent.com/render/math?math=\mathcal{G}_{RASSEL} = \{\widehat{g}^{(1)},...,\widehat{g}^{(l)},...,\widehat{g}^{(L)}\}$$ and forming the ensemble prediction function as: $$\widehat{f}^{(L)}(.) = \dfrac{1}{L}\sum_{l=1}^{L}\widehat{g}^{(l)}(.))
