...
<img src="https://latex.codecogs.com/svg.image?\widehat{f}^{(L)}(.) = \dfrac{1}{L}\sum_{l=1}^{L}\widehat{g}^{(l)}(.)&space;" />

## PREDICTION TASK
### For classification
In classification, we predict the class membership of $x^{*} \in \mathcal{X}$ by using the ensemble predicting estimator:
<img src="https://latex.codecogs.com/svg.image?\widehat{f}^{(L)}(x^{*}) = \underset{y\in\mathcal{Y}}{argmax}\biggl\{\sum_{i=1}^{L}\bigg(\textbf{1}_{(y=\widehat{g}^{(L)}(x^{*}))}\bigg) \biggr\}&space;" />

### For regression
Given <img src="https://latex.codecogs.com/svg.image?x^{*} \in \mathcal{X}&space;" /> , we predict its corresponding response using:
<img src="https://latex.codecogs.com/svg.image?\widehat{f}^{(L)}(x^{*}) = \dfrac{1}{L}\sum_{l=1}^{L}\widehat{g}^{(l)}(x^{*})&space;" />
